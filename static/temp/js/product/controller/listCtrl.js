//产品列表
productApp.controller('listCtrl', ['$scope', '$http', '$state', 'utils', '$stateParams', '$location', '$modal', 'platformModalSvc', 'platformTemplatePreviewSvc',
			...
			if (isLink == true) {
				...
				if (ctg0 != "") {
					...
					$http.get('/pccms/productCtg/pageTplType/' + ctg0.id + '?pageTplType=DETAIL')
						.success(function (data, status, headers, config) {
							if (data.isSuccess) {
								platformTemplatePreviewSvc.previewDetail('product/preview/{id}', id);
							} else {
								platformModalSvc.showWarmingMessage(data.data,'提示');
							}
						})
