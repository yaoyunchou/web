...
pageApp.controller('pageCtrl', ['$scope','$http','$state','utils','platformModalSvc','platformTemplatePreviewSvc',
	function ($scope, $http, $state,utils,platformModalSvc, previewSvc) {
		...
		$scope.copyCreate = function copyCreate(srcTpl){
			$http.put(globals.basAppRoot + 'pageTpl/design/copy/'+srcTpl._id).then(function(res){
				if(res.data.isSuccess){
					platformModalSvc.showSuccessTip(res.data.data);
					loadTree();
				}else{
					platformModalSvc.showWarmingMessage(res.data.data,nsw.Constant.TIP);
				}
			},function(error){
				platformModalSvc.showWarmingMessage(error.status + '-' + status.statusText , nsw.Constant.TIP);
			});
		};
		...
		$scope.blkEdit = function(tplId){
			previewSvc.preview(tplId,'edit',false);
		}
		//项目页面预览
		$scope.singlePagePreview = function(tplId){
			var uri = "pageTpl/design/"+tplId+"/view?isPubTpl=false";
			previewSvc.preview(tplId,'view',false);
		};

	}]);

//页面模板筛选
pageApp.controller('filterCtrl', ['$scope', '$state', '$stateParams', '$http','utils','platformModalSvc','mobilePreviewSvc','desktopMainSvc',
	function ($scope, $state, $stateParams, $http, utils,platformModalSvc,mobilePreviewSvc, desktopMainSvc) {
		...
		//删除单页模版
		$scope.singleTplRemove = function(singleTplId){
			platformModalSvc.showConfirmMessage('确认删除当前模版吗？','提示',true).then(function(){
				$http({
					method: 'DELETE',
					url: globals.basAppRoot + '/tpl/page/' + singleTplId
				}).then(function (res) {
					if(res.data.isSuccess){
						_.remove($scope.filterData.list,{_id:singleTplId});
						platformModalSvc.showSuccessTip(res.data.data);
					}else{
		...
		$scope.selSingle = function(singleTplId){
			var dat = {"tplId":singleTplId , "nodeId":$stateParams.id, "style":$scope.style };
			$http.post('/pccms/proj/projPageTplPack/isReplaceStaticFile',dat).then(function(data) {
				if(data.data.isSuccess){
					var dataTip = data.data.data;
					if(dataTip.length > 0){
						dataTip = data.data.data+'将被替换,';
					}else{
						dataTip = '';
					}
		...
		$scope.selPack = function(id){
			var dat = {"packId": id};
			$http.post('/pccms/proj/projPageTplPack/isReplacePackFile',dat).then(function(data) {
				if(data.data.isSuccess){
					var dataTip = data.data.data;
					if(dataTip.length > 0){
						dataTip = data.data.data+'将被替换,';
					...
					platformModalSvc.showConfirmMessage(dataTip + '确认使用吗？',nsw.Constant.TIP,true).then(function(){
						$http.post('/pccms/proj/projPageTplPack/', dat).then(function(data) {
							if(data.data.isSuccess){
								$state.go('page');
							}else{
								platformModalSvc.showWarmingMessage(nsw.Constant.OPERATION,nsw.Constant.TIP);
		...
		$scope.getIsSelectedClass = function getIsSelectedClass(type){
			$scope.num = $scope.projectType;
			if(!$scope.projectType) {
				desktopMainSvc.getProjectType().then(function (projectType) {
					$scope.projectType = projectType;
				});
			}
			if(type===$scope.projectType){
				return 'selected';
			}
		};
...
//单页面预览
pageApp.controller('previewCtrl', ['$scope','$http','$state','$stateParams','utils','platformModalSvc', 'platformTemplatePreviewSvc',
